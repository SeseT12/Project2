...
class ImageCaptcha:
    ...
    def _draw_character_with_bounding_box(
            self,
            c: str,
            draw: ImageDraw,
            color: ColorTuple) -> (Image, Image):
        font = random.choice(self.truefonts)
        _, _, w, h = draw.multiline_textbbox((1, 1), c, font=font)
        ...
        dy1 = random.randint(0, 6)
        im = createImage('RGBA', (w + dx1, h + dy1))
        #im_bb = createImage('RGBA', (w + dx1, h + dy1))
        Draw(im).text((dx1, dy1), c, font=font, fill=color)
        #Draw(im_bb).text((dx1, dy1), c, font=font, fill=color)

        # rotate
        im = im.crop(im.getbbox())
        #im_bb = im_bb.crop(im_bb.getbbox())
        #Draw(im_bb).rectangle([0, 0, im_bb.size[0] - 1, im_bb.size[1] - 1], outline='Red')

        rotation = random.uniform(-30, 30)
        im = im.rotate(rotation, BILINEAR, expand=True)
        #im_bb = im_bb.rotate(rotation, BILINEAR, expand=True)
    ...
    def create_captcha_image_and_bounding_box(
            self,
            chars: str,
            color: ColorTuple,
        ...
        width = max(text_width, self._width)
        image = image.resize((width, self._height))
        image_with_bb = image_with_bb.resize((width, self._height))
